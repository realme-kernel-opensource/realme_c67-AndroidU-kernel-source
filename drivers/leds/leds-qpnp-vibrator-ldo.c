...
static inline int qpnp_vib_ldo_enable(struct vib_ldo_chip *chip, bool enable)
{
	...
	pr_debug("[%s] vib_type[%d] enable[%d]\n",__func__, chip->type ,enable);
	if (VIB_TYPE_GPIO == chip->type) {
		if (enable)
...
static ssize_t oplus_vib_show_state(struct device *dev,
...
static ssize_t oplus_vib_store_state(struct device *dev,
...
static ssize_t oplus_vib_show_duration(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct vib_ldo_chip *chip = container_of(cdev, struct vib_ldo_chip,
...
static ssize_t oplus_vib_store_duration(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct vib_ldo_chip *chip = container_of(cdev, struct vib_ldo_chip,
...
static ssize_t oplus_vib_show_activate(struct device *dev,
		struct device_attribute *attr, char *buf)
{
...
static ssize_t oplus_vib_store_activate(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct vib_ldo_chip *chip = container_of(cdev, struct vib_ldo_chip,
...
static ssize_t oplus_vib_show_vmax(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct vib_ldo_chip *chip = container_of(cdev, struct vib_ldo_chip,
...
static ssize_t oplus_vib_store_vmax(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct led_classdev *cdev = dev_get_drvdata(dev);
	struct vib_ldo_chip *chip = container_of(cdev, struct vib_ldo_chip,
...
static struct device_attribute oplus_vib_attrs[] = {
	__ATTR(oplus_state, 0664, oplus_vib_show_state, oplus_vib_store_state),
	__ATTR(oplus_duration, 0664, oplus_vib_show_duration, oplus_vib_store_duration),
	__ATTR(oplus_activate, 0664, oplus_vib_show_activate, oplus_vib_store_activate),
	__ATTR(oplus_vmax_mv, 0664, oplus_vib_show_vmax, oplus_vib_store_vmax),
};
...
static int qpnp_vibrator_ldo_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(oplus_vib_attrs); i++) {
		ret = sysfs_create_file(&chip->cdev.dev->kobj,
				&oplus_vib_attrs[i].attr);
		if (ret < 0) {
