...
static ssize_t proc_reserve_soc_debug_write(struct file *filp,
		const char __user *buf, size_t len, loff_t *data)
{
	struct oplus_chg_chip *chip = filp->private_data;
	#define BUFFER_MAX_LEN 4
	...
	if (copy_from_user(buffer, buf, BUFFER_MAX_LEN)) {
		chg_err("copy_from_user error.\n");
		return -EFAULT;
	}
	buffer[BUFFER_MAX_LEN] = '\0';

	if (sscanf(buffer, "%d,%d", &reserve_chg_soc, &reserve_dis_soc) == 2) {
...
static ssize_t proc_reserve_soc_debug_read(struct file *filp,
		char __user *buff, size_t count, loff_t *off)
{
	struct oplus_chg_chip *chip = filp->private_data;
	char page[256] = {0};
	...
	len = sprintf(page, "%d,%d", chip->rsd.reserve_chg_soc,
	...
	struct oplus_chg_chip *chip, int vbat_cell_max, int vbat_cell_min, int status)
{
	...
	index += snprintf(&(chip->vbatt_diff_over_load_trigger.crux_info[index]),
			...
			"$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d"
			"$$vbatt_diff@@%d$$charger_exist@@%d",
			chip->soc, chip->ui_soc, vbat_cell_max,
	...
	struct oplus_chg_chip *chip)
{
	...
	index += snprintf(&(chip->vbatt_too_low_load_trigger.crux_info[index]),
			OPLUS_CHG_TRACK_CURX_INFO_LEN - index, "$$soc@@%d"
			"$$uisoc@@%d$$vbatt_max@@%d$$vbatt_min@@%d",
			chip->soc, chip->ui_soc, chip->batt_volt_max,
			chip->batt_volt_min);
...
static void oplus_chg_cool_down_match_err_check(struct oplus_chg_chip *chip)
{
	...
	if ((chip->charger_volt * 10) / (chip->batt_volt * 10) >= VOLTAGE_RATIO)
		scale = 2;
	else
		scale = 1;
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	if (chip->rsd.smooth_switch_v2) {
		...
		} else if (chip->rsd.reserve_chg_soc < RESERVE_SOC_MIN
		    || chip->rsd.reserve_chg_soc > RESERVE_SOC_MAX) {
			chg_err("oplus,reserve_chg_soc %d invalid rc=%d\n",
				chip->rsd.reserve_chg_soc, rc);
		...
		} else if (chip->rsd.reserve_dis_soc < RESERVE_SOC_MIN
		    || chip->rsd.reserve_dis_soc > RESERVE_SOC_MAX) {
			chg_err("oplus,reserve_dis_soc %d invalid rc=%d\n",
				chip->rsd.reserve_dis_soc, rc);
