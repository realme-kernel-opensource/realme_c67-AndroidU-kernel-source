...
static bool bq27541_hw_config(struct chip_bq27541 *chip)
{
	int ret = 0;
	int flags = 0;
	int device_type = 0;
	...
	if (device_type == DEVICE_TYPE_BQ27411 || device_type == DEVICE_TYPE_BQ27426) {
	...
	dev_err(chip->dev, "DEVICE_TYPE is 0x%02X, FIRMWARE_VERSION is 0x%02X batt_zy0603 = %d\n", device_type, fw_ver,
		chip->batt_zy0603);
	if (device_type != DEVICE_TYPE_BQ27541 && device_type != DEVICE_TYPE_BQ27411
		&& device_type != DEVICE_TYPE_BQ27426 && device_type != DEVICE_TYPE_BQ28Z610
		&& device_type != DEVICE_TYPE_ZY0602 && device_type != DEVICE_TYPE_ZY0603) {
		dev_err(chip->dev, "gauge init fail\n");
		return false;
	}
...
rerun:
	rerun_num--;
	ret = bq27541_hw_config(fg_ic);
	if (!ret) {
		dev_err(&client->dev, "bq27541 config fail, probe fail\n");
		return -ENOMEM;
	}
