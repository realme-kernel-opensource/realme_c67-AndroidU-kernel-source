...
#include "../../../../../kernel_platform/msm-kernel/drivers/usb/typec/tcpc/inc/tcpci.h"
#include "../../../../../kernel_platform/msm-kernel/drivers/usb/typec/tcpc/inc/tcpm.h"
...
static int quiet_thermal_temp = 25000;
...
int oplus_get_cc_value(void)
{
	struct tcpc_device *tcpc = tcpc_dev_get_by_name("type_c_port0");
	if (tcpc == NULL) {
		pr_err("tcpc_dev is null return\n");
		return -1;
	}
	pr_info("cc1=%d cc2=%d\n", tcpc->typec_remote_cc[0], tcpc->typec_remote_cc[1]);

	return tcpc->typec_remote_cc[0] && tcpc->typec_remote_cc[1];
}
...
static int oplus_parse_dt_adc_channels(struct smb_charger *chg)
{
	...
	if (rc < 0)
		pr_err(" %s chgid_v_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "usbtemp_r_v_chan", &chg->iio.usbtemp_r_v_chan);
	if (rc < 0)
		pr_err(" %s usbtemp_r_v_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "usbtemp_l_v_chan", &chg->iio.usbtemp_l_v_chan);
	if (rc < 0)
		pr_err(" %s usbtemp_l_v_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "batbtb_temp_chan", &chg->iio.batbtb_temp_chan);
	if (rc < 0)
		pr_err(" %s ntc_swibatbtb_temp_chantch2_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "usbbtb_temp_chan", &chg->iio.usbbtb_temp_chan);
	if (rc < 0)
		pr_err(" %s usbbtb_temp_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "subboard_temp_chan", &chg->iio.subboard_temp_chan);
	if (rc < 0)
		pr_err(" %s subboard_temp_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "ntc_switch2_chan", &chg->iio.ntc_switch2_chan);
	if (rc < 0)
		pr_err(" %s ntc_switch2_chan get failed\n", __func__);

 	rc = oplus_get_iio_channel(chg, "ntc_switch3_chan", &chg->iio.ntc_switch3_chan);
	if (rc < 0)
		pr_err(" %s ntc_switch3_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "ntc_switch4_chan", &chg->iio.ntc_switch4_chan);
	if (rc < 0)
		pr_err(" %s ntc_switch4_chan get failed\n", __func__);

	rc = oplus_get_iio_channel(chg, "batt_id_chan", &chg->iio.batt_id_chan);
	if (rc < 0)
		pr_err(" %s batt_id_chan get failed\n", __func__);
...
int oplus_chg_get_battery_btb_temp_cal(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chip->chg_ops->get_cp_tsbat && chg->batt_btb_support) {
...
static int oplus_get_ntc_tmp(struct iio_channel *channel)
{
	...
	if (!opluschg) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: discrete not ready!\n", __func__);
		return 25;
	}
...
static int oplus_thermal_get_tmp(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (!chg->ntcswitch_pinctrl
		|| !chg->iio.ntc_switch2_chan) {
	...
	ntcctrl_gpio_value = gpio_get_value(chg->ntcctrl_gpio);
	if (ntcctrl_gpio_value == 0) {
		batt_thermal_temp = oplus_get_ntc_tmp(chg->iio.ntc_switch2_chan);
		pinctrl_select_state(chg->ntcswitch_pinctrl, chg->ntc_switch_high);
		msleep(100);
		ret = gpio_get_value(chg->ntcctrl_gpio);
		flash_thermal_temp = oplus_get_ntc_tmp(chg->iio.ntc_switch2_chan);
	} else if (ntcctrl_gpio_value == 1) {
		flash_thermal_temp = oplus_get_ntc_tmp(chg->iio.ntc_switch2_chan);
		pinctrl_select_state(chg->ntcswitch_pinctrl, chg->ntc_switch_low);
		msleep(100);
		ret = gpio_get_value(chg->ntcctrl_gpio);
		batt_thermal_temp = oplus_get_ntc_tmp(chg->iio.ntc_switch2_chan);
	} else {
	...
	pr_info("%s batt:%d, flash:%d\n", __func__, batt_thermal_temp, flash_thermal_temp);
	return 0;
}
...
static int oplus_thermal_amux_get_tmp(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
  	if (!chg->ntcswitch_pinctrl || !chg->iio.ntc_switch3_chan || !chg->iio.ntc_switch4_chan) {
  	...
  	ntcctrl_gpio_value = gpio_get_value(chg->ntcctrl_gpio_amux);
  	if (ntcctrl_gpio_value == 0) {
  		bb_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch3_chan);
  		chg_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch4_chan);
  		pinctrl_select_state(chg->ntcswitch_pinctrl, chg->ntc_switch_amux_high);
  		msleep(100);
  		ret = gpio_get_value(chg->ntcctrl_gpio_amux);
  		pa_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch3_chan);
  		quiet_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch4_chan);
  	} else if (ntcctrl_gpio_value == 1) {
  		pa_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch3_chan);
  		quiet_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch4_chan);
  		pinctrl_select_state(chg->ntcswitch_pinctrl, chg->ntc_switch_amux_low);
  		msleep(100);
  		ret = gpio_get_value(chg->ntcctrl_gpio_amux);
  		bb_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch3_chan);
  		chg_thermal_temp = oplus_get_ntc_tmp_amux(chg->iio.ntc_switch4_chan);
	...
	pr_info("%s bb:%d, chg:%d, pa:%d, quiet:%d\n", __func__, bb_thermal_temp, chg_thermal_temp, pa_thermal_temp, quiet_thermal_temp);
  	return 0;
}
...
int oplus_thermal_tmp_get_batt(void)
{
	...
	if (!chip) {
		printk(KERN_ERR "[OPLUS_CHG][%s]: discrete_charger not ready!\n", __func__);
		return NTC_DEFAULT_TEMP;
	}

	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->batt_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, batt_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}
...
int oplus_thermal_tmp_get_flash(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->flash_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, flash_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}
...
int oplus_thermal_tmp_get_pa(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->pa_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_amux_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, pa_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}
...
int oplus_thermal_tmp_get_bb(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->bb_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_amux_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, bb_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}
...
int oplus_thermal_tmp_get_quiet(void)
{
	struct oplus_chg_chip *chip = g_oplus_chip;
	struct smb_charger *chg = NULL;
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->quiet_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_amux_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, quiet_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}

	return quiet_thermal_temp;
}
EXPORT_SYMBOL(oplus_thermal_tmp_get_quiet);
...
int oplus_thermal_tmp_get_chg(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (chg && chg->charger_thermal_support) {
		mutex_lock(&chg->ntc_lock);
		oplus_thermal_amux_get_tmp();
		mutex_unlock(&chg->ntc_lock);
	} else {
		pr_info("%s, charger_thermal not support\n", __func__);
		return NTC_DEFAULT_TEMP;
	}
...
static int oplus_ntc_switch_gpio_init(struct oplus_chg_chip *chip)
{
	struct smb_charger *chg = &chip->pmic_spmi.smb5_chip->chg;
	...
	chg->ntcswitch_pinctrl = devm_pinctrl_get(chip->dev);
	if (IS_ERR_OR_NULL(chg->ntcswitch_pinctrl)) {
		pr_info("get ntc_switch_gpio pinctrl fail\n");
		return -EINVAL;
	}

	chg->ntc_switch_high =
			pinctrl_lookup_state(chg->ntcswitch_pinctrl,
			"ntc_switch_high");
	if (IS_ERR_OR_NULL(chg->ntc_switch_high)) {
	...
	chg->ntc_switch_low =
			pinctrl_lookup_state(chg->ntcswitch_pinctrl,
			"ntc_switch_low");
	if (IS_ERR_OR_NULL(chg->ntc_switch_low)) {
		pr_info("get ntc_switch_low fail\n");
		return -EINVAL;
	}

	/*default switch chg_thermal and batt_thermal_temp*/
	pinctrl_select_state(chg->ntcswitch_pinctrl, chg->ntc_switch_low);

	if (is_double_ntc_switch) {
  		chg->ntc_switch_amux_high = pinctrl_lookup_state(chg->ntcswitch_pinctrl, "ntc_switch_amux_high");
  		if (IS_ERR_OR_NULL(chg->ntc_switch_amux_high)) {
  			chg_err("get ntc_switch_amux_high fail\n");
  			return -EINVAL;
  		}
  
  		chg->ntc_switch_amux_low = pinctrl_lookup_state(chg->ntcswitch_pinctrl, "ntc_switch_amux_low");
  		if (IS_ERR_OR_NULL(chg->ntc_switch_amux_low)) {
...
static int oplus_ntc_switch_parse_dt(struct oplus_chg_chip *chip)
{
	...
	chg->ntcctrl_gpio = of_get_named_gpio(node, "qcom,ntc-switch-gpio", 0);
	chg->ntcctrl_gpio_amux = of_get_named_gpio(node, "qcom,ntc-switch-gpio-amux", 0);
	if (chg->ntcctrl_gpio < 0) {
		pr_err("chg->ntcctrl_gpio not specified\n");
	} else {
		if (gpio_is_valid(chg->ntcctrl_gpio)) {
			rc = gpio_request(chg->ntcctrl_gpio,
				"ntc-switch-gpio");
			if (rc) {
				pr_err("unable to request gpio [%d]\n",
					chg->ntcctrl_gpio);
			} else {
				rc = oplus_ntc_switch_gpio_init(chip);
				if (rc)
					chg_err("unable to init charging_sw_ctrl2-gpio:%d\n",
							chg->ntcctrl_gpio);
			}
		}
		pr_info("chg->ntcctrl_gpio =%d\n", chg->ntcctrl_gpio);
	}

	if (chg->ntcctrl_gpio_amux < 0) {
  		chg_err("chip->ntcctrl_gpio_amux not specified\n");
  	} else {
  		if (gpio_is_valid(chg->ntcctrl_gpio_amux)) {
  			rc = gpio_request(chg->ntcctrl_gpio_amux, "ntc-switch-gpio_amux");
  			is_double_ntc_switch = true;
  			if (rc) {
  				chg_err("unable to request gpio [%d]\n",
  				chg->ntcctrl_gpio_amux);
  			} else {
  				rc = oplus_ntc_switch_gpio_init(chip);
  				if (rc)
  		...
  		chg_info("chg->ntcctrl_gpio_amux =%d\n", chg->ntcctrl_gpio_amux);
  	}
...
#define NTC_DEFAULT_VOLT_VALUE_MV 950
#define BATID_LOW_MV1 70
#define BATID_HIGH_MV1 180
#define BATID_LOW_MV2 550
#define BATID_HIGH_MV2 790
#define NUM_0 0
#define NUM_1 1
#define ERR_NUM -1
enum {
...
bool battery_type_check(void)
{
	...
	chg = &chip->pmic_spmi.smb5_chip->chg;
	if (!chg->iio.batt_id_chan) {
		chg_err("[OPLUS_CHG][%s]: chip not ready!\n", __func__);
		ret = oplus_get_iio_channel(chg, "batt_id_chan", &chg->iio.batt_id_chan);
		if (ret < NUM_0 || !chg->iio.batt_id_chan) {
			chg_err(" %s batt_id_chan get failed\n", __func__);
			return true;
		}
	}

	ret = iio_read_channel_processed(chg->iio.batt_id_chan, &value);
	if (ret < NUM_0) {
	...
	} else if (value >= BATID_LOW_MV2 && (value <= BATID_HIGH_MV2)) {
		battery_type = BAT_TYPE_ATL;
	}

	chg_err("battery_id := %d, battery_type:%d\n", value, battery_type);
	if (battery_type == BAT_TYPE_LIWINON ||
	    battery_type == BAT_TYPE_ATL) {
		return true;
	} else {
		return false;
...
static int oplus_chg_parse_custom_dt(struct oplus_chg_chip *chip)
{
	...
	if(g_oplus_chip) {
		...
		chg->usb_btb_support = of_property_read_bool(node, "qcom,usb_btb_support");
		chg->batt_btb_support = of_property_read_bool(node, "qcom,batt_btb_support");
		chg->pa_thermal_support = of_property_read_bool(node, "qcom,pa_thermal_support");
		chg->batt_thermal_support = of_property_read_bool(node, "qcom,batt_thermal_support");
		chg->bb_thermal_support = of_property_read_bool(node, "qcom,bb_thermal_support");
		chg->charger_thermal_support = of_property_read_bool(node, "qcom,charger_thermal_support");
		chg->quiet_thermal_support = of_property_read_bool(node, "qcom,quiet_thermal_support");
		chg->flash_thermal_support = of_property_read_bool(node, "qcom,flash_thermal_support");
	}
#endif
	return rc;
}
...
int oplus_get_adapter_svid(void)
{
	...
	for (i = 0; i < svid_list.cnt; i++) {
		pr_err("oplus_get_adapter_svid:svid[%d] = 0x%x\n", i, svid_list.svids[i]);
	...
	if ((vdos[0] & 0xFFFF) == OPLUS_SVID) {
		g_oplus_chip->pd_svooc = true;
		pr_err("oplus_get_adapter_svid:match svid and this is oplus adapter 11\n");
		...
		pr_err("oplus_get_adapter_svid:svid[0x%x],pid&bcd [0x%x 0x%x]", vdos[0], vdos[1], vdos[2]);
	}
	...
	if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		while (1) {
			...
			if (ret == TCPM_ERROR_NOT_FOUND) {
				...
				pr_err("tcpm_inquire_pd_source_apdo failed(%d)", ret);
				break;
			}

			ret = tcpm_dpm_pd_get_source_cap_ext(chg->tcpc,
				NULL, &cap_ext);
			if (ret == TCPM_SUCCESS)
				chg->srccap.pdp = cap_ext.source_pdp;
			else {
				chg->srccap.pdp = 0;
				pr_err("tcpm_dpm_pd_get_source_cap_ext failed(%d)", ret);
			}

			chg->srccap.pwr_limit[idx] = apdo_cap.pwr_limit;
			/* If TA has PDP, we set pwr_limit as true */
			if (chg->srccap.pdp > 0 && !chg->srccap.pwr_limit[idx])
				chg->srccap.pwr_limit[idx] = 1;
			chg->srccap.ma[idx] = apdo_cap.ma;
			chg->srccap.max_mv[idx] = apdo_cap.max_mv;
			chg->srccap.min_mv[idx] = apdo_cap.min_mv;
			chg->srccap.maxwatt[idx] = apdo_cap.max_mv * apdo_cap.ma;
			chg->srccap.minwatt[idx] = apdo_cap.min_mv * apdo_cap.ma;
			//chg->srccap.type[idx] = MTK_PD_APDO;
			...
			if (idx >= ADAPTER_CAP_MAX_NR) {
				pr_err("CAP NR > %d", ADAPTER_CAP_MAX_NR);
				break;
			}
		}
		...
		for (i = 0; i < chg->srccap.nr; i++) {
			pr_err("pps_cap[%d:%d], %d mv ~ %d mv, %d ma pl:%d pdp:%d",
				i, (int)chg->srccap.nr, chg->srccap.min_mv[i],
				chg->srccap.max_mv[i], chg->srccap.ma[i],
				chg->srccap.pwr_limit[i], chg->srccap.pdp);
		}

		if (cap_i == 0 || chg->srccap.nr == 0)
			pr_err("no APDO for pps");
		else
			oplus_pps_set_power(OPLUS_PPS_POWER_THIRD,
				chg->srccap.ma[chg->srccap.nr - 1],
				chg->srccap.max_mv[chg->srccap.nr - 1]);
	}
...
int oplus_chg_pps_get_max_cur(int vbus_mv)
{
	...
	if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		for (i = 0; i < chg->srccap.nr; i++) {
			if (chg->srccap.min_mv[i] <= vbus_mv && vbus_mv <= chg->srccap.max_mv[i]) {
				if (ibus_ma < chg->srccap.ma[i]) {
					ibus_ma = chg->srccap.ma[i];
				}
			}
		}
	}
...
int oplus_chg_pps_get_max_volt(void)
{
	...
	if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
		for (i = 0; i < chg->srccap.nr; i++) {
			if (vbus_mv < chg->srccap.max_mv[i]) {
				vbus_mv = chg->srccap.max_mv[i];
			}
		}
	}
...
int oplus_chg_get_charger_subtype(void)
{
	...
	else if (chg->pd_active == QTI_POWER_SUPPLY_PD_PPS_ACTIVE) {
...
static int discrete_charger_probe(struct platform_device *pdev)
{
	...
	oplus_ntc_switch_parse_dt(oplus_chip);
