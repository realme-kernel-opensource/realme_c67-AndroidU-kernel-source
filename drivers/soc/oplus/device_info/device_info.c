...
static int devinfo_read_emmc_func(struct seq_file *s, void *v)
{
	...
	switch (mmc->card->cid.manfid) {
		...
		case 0x9b:
			...
			break;
		default:
			printk("%s unknown card->cid.manfid is %x\n", __func__, mmc->card->cid.manfid);
			manufacture = "Unknown";
	}
...
static int devinfo_read_emmcfw_func(struct seq_file *s, void *v)
{
	...
	sprintf(temp_version, "0x%02x,0x%02x%02x", mmc->card->cid.prv,
  ...
  static int __attribute__((__unused__)) init_ddr_vendor_size(struct device_info *dev_info)
  {
	...
	uint32_t ddr_id = 0;
  	...
  	/*if (strlen(ddr_vendor_size) != 0) {
        ...
        ddr_size =  of_fdt_get_ddrsize();
	
	ddr_id = of_fdt_get_ddrid();
  
	switch(ddr_id) {
		case 1:
			memcpy(ddr_manufacture, "SAMSUNG", sizeof("SAMSUNG"));
			break;
		case 6:
			memcpy(ddr_manufacture, "SKHYNIX", sizeof("SKHYNIX"));
			break;
		case 255:
			memcpy(ddr_manufacture, "MICRON", sizeof("MICRON"));
			break;
		case 19:
			memcpy(ddr_manufacture, "CXMT", sizeof("CXMT"));
			break;
		default:
			memcpy(ddr_manufacture, "UNKNOWN", sizeof("UNKNOWN"));
			break;
	}
  	/*if (strcmp(ddr_id, "1") == 0) {
  			memset(ddr_manufacture, 0, sizeof(ddr_manufacture));
  			strcpy(ddr_manufacture, "Samsung");
  		} else if (strcmp(ddr_id, "6") == 0) {
  			memset(ddr_manufacture, 0, sizeof(ddr_manufacture));
  			strcpy(ddr_manufacture, "Hynix");
  		} else if (strcmp(ddr_id, "19") == 0) {
  			memset(ddr_manufacture, 0, sizeof(ddr_manufacture));
  			strcpy(ddr_manufacture, "Cxmt");
  		} else if (strcmp(ddr_id, "255") == 0) {
  			memset(ddr_manufacture, 0, sizeof(ddr_manufacture));
  			strcpy(ddr_manufacture, "Micron");
  		} else {
  			memset(ddr_manufacture, 0, sizeof(ddr_manufacture));
  		...
  		sprintf(ddr_manufacture, "%s|%dG", ddr_manufacture, ddr_size);
  		memcpy(info->manufacture, ddr_manufacture, strlen(ddr_manufacture) > 31?31:strlen(ddr_manufacture));
  		pr_err("device_info.vendor_size= %s\n", ddr_manufacture);
  //	}
