...
#define WAKELOCK_HOLD_IRQ_TIME 3000//500 /* in ms */
#define WAKELOCK_HOLD_CMD_TIME 1000 /* in ms */
#define WAKELOCK_HOLD_CMD_TIME_35 (35*1000) /* in ms */
...
static long fp_ioctl(struct file *filp, unsigned int cmd, unsigned long arg) {
    ...
    switch (cmd) {
        ...
        case FP_IOC_WAKELOCK_TIMEOUT_ENABLE:
            ...
            break;
        case FP_IOC_WAKELOCK_TIMEOUT_35_ENABLE:
            pr_info("%s FP_IOC_WAKELOCK_TIMEOUT_35_ENABLE\n", __func__);
            wake_lock_timeout(&fp_cmd_wakelock, msecs_to_jiffies(WAKELOCK_HOLD_CMD_TIME_35));
            break;
